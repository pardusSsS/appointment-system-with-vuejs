...
class ReminderTask extends Command
{
    ...
    public function handle()
    {
        ...
        $list = Appointment::whereIn('date',[$date,$nextDate])->where('isActive',APPOINTMENT_SUCCESS)->where('isSend',REMINDER_DEFAULT)->get();
        ...
        foreach ($list as $k => $v){
            ...
            if($v['notification_type']==NOTIFICATION_EMAIL){
                ...
                Mail::send('email',$data,function ($message) use ($data){
                  ...
                  $message->from('omeraksit0550@gmail.com','Ömer Akşit');
                });
            ...
            if($v['notification_type']==NOTIFICATION_SMS){
                ...
                $message='Merhaba '.$v['fullName'].', '.$v['date'].' tarihinde '.WorkingHours::getString($v['workingHour']).' saatleri arasında randevunuz olduğunu hatrlatmaktan memnuniyet duyarız. Takip kodunuz:' .$v['code'];
